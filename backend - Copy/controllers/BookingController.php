                    ...
                    foreach ($booking_menu as $value) {
                        ...
                        if ($menu_data) {
                            ...
                            foreach ($menu_items as $items) {
                                $itemsAll['item_id'] = $items;
                                $itemsAll['menu_id'] = ($menu_data->gujarati) ? $menu_data->id . '- ' . $menu_data->english . ' (' . $menu_data->gujarati . ')' : $menu_data->id . '- ' . $menu_data->english;
                                $itemsIdArray[] = $itemsAll;
                            }
                        }
                    }
                    $tmp = array();
                    if ($itemsIdArray) {
                        foreach ($itemsIdArray as $arg) {
                            $tmp[$arg['item_id']][] = $arg['menu_id'];
                        }
                    }
                    $unique_itemsId = array_keys($tmp);
                    $unique_itemsId = array_values(array_unique($unique_itemsId));
                    if ($unique_itemsId) {
                        foreach ($unique_itemsId as $items) {
                            $items_data = Items::find()->where(['id' => $items])->one();
                            if ($items_data) {
                                $BookingItemsModel_update = BookingItems::find()->where(['booking_id' => $model->id, 'item_id' => $items_data->id])->count();
                                if (empty($BookingItemsModel_update)) {
                                    ...
                                    $BookingItemsModel->menu_id = (isset($tmp[$items]) && $tmp[$items]) ? implode(',', $tmp[$items]) : '';
                                    ...
                                    if (!$BookingItemsModel->save()) {
                                        echo '<pre>';
                                        print_r($BookingItemsModel->getErrors());
                                        echo '</pre>';
                                        exit;
                                    }
                                }
            ...
            if ($model->load(Yii::$app->request->post())) {
                ...
                if ($beforeMenu) {
                    foreach ($beforeMenu as $bvalue) {
                        if ($postdata['Booking']['menu'] && !in_array($bvalue, $postdata['Booking']['menu'])) {
                            BookingItems::updateAll(['menu_id' => ''], ['booking_id' => $model->id]);
                        }
                    }
                }
                $model->menu = ($postdata['Booking']['menu']) ? implode(",", $postdata['Booking']['menu']) : '';
                ...
                if ($booking_menu) {
                    ...
                    if ($unique_itemsId) {
                        foreach ($unique_itemsId as $items) {
                            $items_data = Items::find()->where(['id' => $items])->one();
                            if ($items_data) {
                                $BookingItemsModel = BookingItems::find()->where(['booking_id' => $model->id, 'item_id' => $items_data->id])->one();
                                ...
                                BookingItems::deleteAll(['booking_id'=>$model->id, 'menu_id'=>'']);
                            }
                        }
                    }
                }
