...
if (!empty(Yii::$app->BackFunctions->checkaccess('create', Yii::$app->controller->id))) {
    $createBtn = Html::a('<i class="fas fa-plus"></i>', FALSE, ['value' => Url::to(['items/create']), 'title' => 'Create item', 'class' => 'showModalButton btn btn-primary']);
}
//Export
$exportBtn = '';
...
//                                    ],
                        [
                            ...
                            'hAlign' => 'left',
                            'format' => 'raw',
                            'filter' => ArrayHelper::map(ItemsCategories::find()->where(['status' => 'Active'])->all(), 'id', function($item){
                                return $item->id.'-'.$item->gujarati.' ('.$item->english.')';
                            }),
                            'filterInputOptions' => ['class' => 'form-select'],
                            'value' => function ($model, $key, $index, $widget) {
                                return $model->itemCategory->id.'-'.$model->itemCategory->gujarati.' ('.$model->itemCategory->english.')';
                            },
                        ],
                        [
                            'attribute' => 'gujarati',
                            ...
                            'format' => 'raw',
                            'value' => function ($model, $key, $index, $widget) {
                                return $model->id.'- '.$model->gujarati.' ('.ucfirst($model->english).')';
                            },
                        ],
                        [
                                ...
                                if (!empty(Yii::$app->BackFunctions->checkaccess('status', Yii::$app->controller->id))) {
                                    return Html::a($model->status,
                                                    ...
                                                    [
                                                        ...
                                                        'title' => $model->status,
                                                        'onclick' => '$.post({
                                                            url: "' . Yii::$app->homeUrl . 'items/statusupdate?id=' . $model->id . '",
                                ...
                                'view' => function ($url, $model) {
                                    if (!empty(Yii::$app->BackFunctions->checkaccess('view', Yii::$app->controller->id))) {
                                        return Html::a(
                                                        ...
                                                        FALSE,
                                                        [
                                                            'value' => Url::to(['items/view', 'id' => $model->id]),
                                                            'title' => 'View Category',
                                ...
                                'update' => function ($url, $model) {
                                    if (!empty(Yii::$app->BackFunctions->checkaccess('update', Yii::$app->controller->id))) {
                                        return Html::a(
                                                        ...
                                                        FALSE,
                                                        [
                                                            'value' => Url::to(['items/update', 'id' => $model->id]),
                                                            'title' => 'Edit Category',
...
<script type="text/javascript">
    function applyjs(e) {
        ...
        if (confirmalert == true) {
            ...
            $.post({
                url: '<?= Yii::$app->homeUrl . 'items/applystatus' ?>',
                dataType: 'json',
