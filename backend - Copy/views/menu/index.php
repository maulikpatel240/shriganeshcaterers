...
if (!empty(Yii::$app->BackFunctions->checkaccess('create', Yii::$app->controller->id))) {
    $createBtn = Html::a('<i class="fas fa-plus"></i>', Url::to(['menu/create'], true), ['value' => Url::to(['menu/create']), 'title' => 'Create menu', 'class' => 'btn btn-primary', 'data-pjax' => 0]);
}
//Export
$exportBtn = '';
        ...
        <div class="row mb-2">
            ...
            <div class="col-sm-6">
                ...
                echo Breadcrumbs::widget([
                    ...
                    'itemTemplate' => '<li class="breadcrumb-menu">{link}</li>', // template for all links
...
//                                    ],
                        [
                            ...
                            'value' => function ($model) {
                                $imagesrc = Html::img(Yii::$app->urlManager->baseUrl . '/uploads/menu/' . $model->image, ['width' => '70', 'height' => '70', 'class' => 'img-circle', 'onerror' => "this.onerror=null;this.src='" . Yii::$app->params['default_error_img'] . "';"]);
                        ...
                        [
                            ...
                            'hAlign' => 'left',
                            'format' => 'raw',
                            'filter' => ArrayHelper::map(Categories::find()->where(['status' => 'Active'])->orderBy(['position' => SORT_ASC, 'gujarati' => SORT_ASC])->asArray()->all(), 'id', function($item) {
                                        return $item['id'] . '-' . $item['gujarati'] . ' (' . $item['english'] . ')';
                                    }),
                            'filterInputOptions' => ['class' => 'form-select'],
                            'value' => function ($model, $key, $index, $widget) {
                                return $model->menuCategory->id.'-'.$model->menuCategory->gujarati. ' (' . $model->menuCategory->english . ')';
                            },
                        ],
                        [
                            'attribute' => 'english',
                            ...
                            'format' => 'raw',
                            'value' => function ($model, $key, $index, $widget) {
                                return $model->id.'- '.ucfirst($model->english);
                            },
                        ],
                        [
                            'attribute' => 'gujarati',
                                ...
                                if (!empty(Yii::$app->BackFunctions->checkaccess('status', Yii::$app->controller->id))) {
                                    return Html::a($model->status,
                                                    ...
                                                    [
                                                        ...
                                                        'title' => $model->status,
                                                        'onclick' => '$.post({
                                                            url: "' . Yii::$app->homeUrl . 'menu/statusupdate?id=' . $model->id . '",
...
<script type="text/javascript">
    function applyjs(e) {
        ...
        if (confirmalert == true) {
            ...
            $.post({
                url: '<?= Yii::$app->homeUrl . 'menu/applystatus' ?>',
                dataType: 'json',
