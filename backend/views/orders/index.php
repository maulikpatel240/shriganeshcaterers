...
if (!empty(Yii::$app->BackFunctions->checkaccess('create', Yii::$app->controller->id))) {
    $createBtn = Html::a('<i class="fas fa-plus"></i>', FALSE, ['value' => Url::to(['orders/create']), 'title' => 'Create orders', 'class' => 'showModalButton btn btn-primary']);
}
//Export
$exportBtn = '';
...
//                                    ],
                        [
                            ...
                            'value' => function ($model, $key, $index, $widget) {
                                if (!empty(Yii::$app->BackFunctions->checkaccess('index', 'booking'))) {
                                    return Html::a(ucwords($model->name), Url::to(['/booking', 'OrdersSearch[id]' => $model->id], true), ['title' => 'View Booking', 'class' => '', 'data-pjax' => 0]);
                                }
                                ...
                                if (!empty(Yii::$app->BackFunctions->checkaccess('status', Yii::$app->controller->id))) {
                                    return Html::a($model->status,
                                                    ...
                                                    [
                                                        ...
                                                        'title' => $model->status,
                                                        'onclick' => '$.post({
                                                            url: "' . Yii::$app->homeUrl . 'orders/statusupdate?id=' . $model->id . '",
                                ...
                                'view' => function ($url, $model) {
                                    if (!empty(Yii::$app->BackFunctions->checkaccess('view', Yii::$app->controller->id))) {
                                        return Html::a(
                                                        ...
                                                        FALSE,
                                                        [
                                                            'value' => Url::to(['orders/view', 'id' => $model->id]),
                                                            'title' => 'View Orders',
                                ...
                                'update' => function ($url, $model) {
                                    if (!empty(Yii::$app->BackFunctions->checkaccess('update', Yii::$app->controller->id))) {
                                        return Html::a(
                                                        ...
                                                        FALSE,
                                                        [
                                                            'value' => Url::to(['orders/update', 'id' => $model->id]),
                                                            'title' => 'Edit Orders',
...
<script type="text/javascript">
    function applyjs(e) {
        ...
        if (confirmalert == true) {
            ...
            $.post({
                url: '<?= Yii::$app->homeUrl . 'orders/applystatus' ?>',
                dataType: 'json',
