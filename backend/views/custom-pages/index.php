...
if (!empty(Yii::$app->BackFunctions->checkaccess('create', Yii::$app->controller->id))) {
    $createBtn = Html::a('<i class="fas fa-plus"></i>', Url::to(['custom-pages/create']), ['title' => 'Create page', 'data-pjax' => 0, 'class' => 'btn btn-primary']);
}
//Export
$exportBtn = '';
...
//                                    ],
                        [
                            ...
                            'value' => function ($model) {
                                $imagesrc = Html::img(Yii::$app->urlManager->baseUrl . '/uploads/custompage/' . $model->image, ['width' => '70', 'height' => '70', 'class' => 'img-circle', 'onerror' => "this.onerror=null;this.src='" . Yii::$app->params['default_error_img'] . "';"]);
                                ...
                                if (!empty(Yii::$app->BackFunctions->checkaccess('status', Yii::$app->controller->id))) {
                                    return Html::a($model->status,
                                                    ...
                                                    [
                                                        ...
                                                        'title' => $model->status,
                                                        'onclick' => '$.post({
                                                            url: "' . Yii::$app->homeUrl . 'custom-pages/statusupdate?id=' . $model->id . '",
                                ...
                                'view' => function ($url, $model) {
                                    if (!empty(Yii::$app->BackFunctions->checkaccess('view', Yii::$app->controller->id))) {
                                        return Html::a(
                                                        ...
                                                        FALSE,
                                                        [
                                                            'value' => Url::to(['custom-pages/view', 'id' => $model->id]),
                    ...
                    'itemLabelSingle' => 'Custom Page',
                    'itemLabelPlural' => 'Custom Pages',
                ]);
                ?>
            </div>
        </div>
...
<script type="text/javascript">
    function applyjs(e) {
        ...
        if (confirmalert == true) {
            ...
            $.post({
                url: '<?= Yii::$app->homeUrl . 'custom-pages/applystatus' ?>',
                dataType: 'json',
